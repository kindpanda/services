...
class ServicesBroker {
  ...
  registerServices(servicesClasses) {
    ...
    this.logger && this.logger.info('Services registered', {
  ...
  async process({
    ...
    const logger = this.logger && this.logger.context({
      service,
      action,
      args,
      exposed
    ...
    if (!this.services.has(service)) {
      ...
      throw (0, _httpErrors.default)(404, 'Unknown service');
    }

    const Service = this.services.get(service);
    const options = { ...this.serviceOptionsFactory(Service),
      exposed: true,
      ...
      services: new Proxy({}, {
        get: (obj1, serviceName) => {
          return new Proxy({}, {
            get: (obj2, actionName) => {
              return (actionArgs = {}) => this.process({
                service: serviceName,
                action: actionName,
                args: actionArgs,
                exposed: false
    ...
    if (!instance[action] || typeof instance[action] !== 'function') {
      ...
      throw (0, _httpErrors.default)(404, 'Unknown action');
    }
    ...
    if (exposed && !expose.includes(action)) {
      ...
      throw (0, _httpErrors.default)(403, 'Unauthorized action');
    }
    ...
    try {
      ...
      logger && logger.info('Dispatch action result', {
        result
      });
      return result;
    } catch (error) {
      logger && logger.error('Dispatching action error', {
        error
      });
      throw error;
    }
